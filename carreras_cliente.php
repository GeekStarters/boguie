...
if((isset($_POST['desde']) && $_POST['desde'] != "") && (isset($_POST['hasta']) && $_POST['hasta'] != "")){
    $sql = "SELECT tbl_ride.passenger, tbl_user.email, tbl_user.fullname, COUNT(tbl_ride.passenger) as carreras from tbl_ride INNER JOIN tbl_user ON tbl_ride.passenger = tbl_user.id WHERE tbl_user.usertype = 'passenger' and tbl_ride.pickup_date BETWEEN '".$_POST["desde"]."' AND '".$_POST['hasta']."' GROUP BY tbl_ride.passenger;";
        ...
        <tbody>
            <?php
             while ($line_raw = ms_stripslashes(mysql_fetch_array($result))){
                ...
                ?>
                <td><a href="carreras_cliente_detail.php?cli=<?=$line_raw['passenger'];?>&d=<?php echo $_POST['desde'] ?>&h=<?php echo $_POST['hasta'] ?>">Ver detalles</a></td>
                <?php
                }else{
                ?>
                <td><a href="carreras_cliente_detail.php?cli=<?=$line_raw['passenger'];?>">Ver detalles</a></td>
                <?php
                }
                ?>
                </tr>
