...
if(isset($_GET['d']) && isset($_GET['h'])){
    $sql = "SELECT carrera.id, carrera.pickup_date, carrera.pickup_time, carrera.pickup_address, cliente.fullname as cliente_name, taxista.fullname as taxista_name from tbl_ride as carrera INNER JOIN tbl_user as cliente ON carrera.passenger = cliente.id LEFT JOIN tbl_user as taxista ON carrera.driver = taxista.id WHERE cliente.usertype = 'passenger' AND carrera.passenger = ".$_GET['cli']." and carrera.pickup_date BETWEEN '".$_GET["d"]."' AND '".$_GET['h']."'";
    
}else{
    $sql = "SELECT carrera.id, carrera.pickup_date, carrera.pickup_time, carrera.pickup_address, cliente.fullname as cliente_name, taxista.fullname as taxista_name from tbl_ride as carrera INNER JOIN tbl_user as cliente ON carrera.passenger = cliente.id LEFT JOIN tbl_user as taxista ON carrera.driver = taxista.id WHERE cliente.usertype = 'passenger' AND carrera.passenger = ".$_GET['cli'];
}
//print_r($sql);
$result = db_query($sql);
?>
