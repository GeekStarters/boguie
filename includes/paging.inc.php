...
if($reccnt>$pagesize){
	...
	<tr>
		...
		<?php for($i=$j;$i<$k;$i++){
			if($i==$j)echo "Page:";
			if(($pagesize*($i))!=$start){?>
				<a href="<?=$PHP_SELF?><?=$qry_str?>&start=<?=$pagesize*($i)?>" style="text-decoration:none;" class="subcat"><?=$i+1?></a>
			<?php }else{?>
				<b><?=$i+1?></b>
			<?php }
		}?></td>
		...
		<?php if($start+$pagesize < $reccnt){?>
			...
			<?php if($start+($pagesize*2)>$reccnt){echo($reccnt-($start+$pagesize));}else{echo($pagesize);}?>&nbsp;&raquo;</a>
		<?php }?></td>
